/*
 * phase3_eval_scenarios.cc
 *
 * Phase-3 ns-3 C++ scenario generator for *unseen* evaluation datasets.
 *
 * Goal
 * ----
 * Generate new (out-of-training-distribution) CSV traces to evaluate:
 * - Phase-2 PPO agent (offline replay)
 * - Baseline algorithm (fixed TTT/HYS or Algorithm1)
 *
 * Output schema matches the Phase-1 generator (dataset.cpp):
 *   <outputPrefix>_tick.csv
 *   <outputPrefix>_events.csv
 *   <outputPrefix>_summary.json
 *
 * Scenarios (unseen during Phase-2 training)
 * ------------------------------------------
 *  8: HetNet Hotspot (macro + dense micro grid) — stability + ping-pong stress
 *  9: Interference Burst Urban Grid — non-stationary RF conditions (time window)
 * 10: Rural High-Speed Corridor with Coverage Hole — RLF / recovery stress
 *
 * Example (from ns-3 root):
...
constexpr uint32_t kRlfT310Ticks = 2;      // 2 * 100ms = 200ms
constexpr uint32_t kTopNeighborsToLog = 6; // n1..n6
...
struct TrajectoryPoint
{
  ...
  double speedMps = 0.0;
  bool turning = false;
  bool inTunnel = false; // used as a generic “coverage hole / deep shadow” flag
};
...
double
Dist2d(const Vector& a, const Vector& b)
{
  double dx = a.x - b.x;
  double dy = a.y - b.y;
...
double
WrapRange(double x, double minV, double maxV)
{
...
double
DistanceToNearestGridLine(double value, double spacing)
{
  if (spacing <= 0.0)
  {
...
std::string
NormalizePatternCode(const std::string& raw)
{
  if (raw.empty())
  {
    return "A";
...
void
AddGnb(std::vector<GnbSpec>& gnbs, double x, double y, double z, double txPowerDbm)
{
  GnbSpec g;
...
Vector
Lerp(const Vector& a, const Vector& b, double t)
{
  return Vector(a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t, a.z + (b.z - a.z) * t);
}

Vector
SampleAlongPolyline(const std::vector<Vector>& points, double distanceM, bool loop)
{
  if (points.empty())
  {
...
TrajectoryPoint
SampleStopGoWaypoints(const std::vector<Vector>& waypoints, double speedMps, double pauseS, double t, double phaseOffsetS)
{
  TrajectoryPoint out;
...
std::map<uint32_t, ScenarioSpec>
BuildScenarioCatalog()
{
  std::map<uint32_t, ScenarioSpec> out;

  // -------------------------------------------------------------------------
  // Scenario 8: HetNet hotspot (macro + micro grid)
  // -------------------------------------------------------------------------
  {
    ScenarioSpec s;
    s.id = 8;
    ...
    // Dense micro grid around the hotspot (8 micros)
  ...
  // -------------------------------------------------------------------------
  // Scenario 9: Urban grid with a time-bounded interference burst
  // -------------------------------------------------------------------------
  {
    ScenarioSpec s;
    s.id = 9;
    ...
    // Two small cells on the center vertical corridor
  ...
  // -------------------------------------------------------------------------
  // Scenario 10: Rural high-speed corridor with a coverage hole
  // -------------------------------------------------------------------------
  {
    ScenarioSpec s;
    s.id = 10;
...
PatternSpec
FindPattern(const ScenarioSpec& scenario, const std::string& rawPattern)
{
...
private:
  ...
  void
  AddBuilding(double x1, double x2, double y1, double y2, double h)
  {
  ...
  void
  GenerateGridBuildings(double blockM, double streetM, double density, double hMin, double hMax)
  {
  ...
  SetupBuildings()
  {
    ...
    {
    case 8:
      // Dense blocks around the hotspot area.
      GenerateGridBuildings(55.0, 25.0, 0.55, 20.0, 40.0);
      break;

    case 9:
      // Manhattan-like blocks.
      GenerateGridBuildings(90.0, 20.0, 0.70, 35.0, 60.0);
      break;

    case 10:
      // Sparse obstacles around the coverage hole region (for visual consistency).
  ...
  // -------------------------------------------------------------------------
  // Trajectory generators (Phase-3 scenarios)
  // -------------------------------------------------------------------------

  TrajectoryPoint
  SampleScenario8(const UeRuntime& ue, double t) const
  {
    TrajectoryPoint tp;
    ...
    if (m_pattern.code == "A")
    {
      // Hotspot perimeter loop (walk).
      double speed = 1.8;
      std::vector<Vector> route = {
        Vector(250.0, 250.0, 1.5),
        Vector(750.0, 250.0, 1.5),
        Vector(750.0, 750.0, 1.5),
        Vector(250.0, 750.0, 1.5),
        Vector(250.0, 250.0, 1.5),
      };
    ...
    if (m_pattern.code == "B")
    {
      ...
      Vector p0(50.0, 500.0, 1.5);
      Vector p1(500.0, 500.0, 1.5);
      Vector p2(950.0, 500.0, 1.5);
      ...
      if (phase < 80.0)
      {
        ...
        tp.position = Vector(500.0 + r * std::sin(0.16 * p + ue.phaseOffsetS),
                             500.0 + r * std::cos(0.16 * p + 1.3 + ue.phaseOffsetS),
                             1.5);
        tp.speedMps = 2.0;
        tp.turning = true;
        return tp;
    ...
    std::vector<Vector> waypoints = {
      ...
      Vector(500.0, 250.0, 1.5),
      Vector(750.0, 500.0, 1.5),
      Vector(500.0, 750.0, 1.5),
      Vector(250.0, 500.0, 1.5),
    };
  ...
  TrajectoryPoint
  SampleScenario9(const UeRuntime& ue, double t) const
  {
    TrajectoryPoint tp;
    double local = t + ue.phaseOffsetS;
    ...
    if (m_pattern.code == "A")
    {
      ...
      std::vector<Vector> route = {
        ...
        Vector(1120.0, 1120.0, 1.5),
        Vector(80.0, 1120.0, 1.5),
        Vector(1120.0, 80.0, 1.5),
        Vector(80.0, 80.0, 1.5),
      };
      tp.position = SampleAlongPolyline(route, speed * local, true);
      tp.speedMps = speed;
    ...
    if (m_pattern.code == "B")
    {
      ...
      std::vector<Vector> route = {
        ...
        Vector(600.0, 600.0, 1.5),
        Vector(60.0, 600.0, 1.5),
        Vector(600.0, 600.0, 1.5),
        Vector(1140.0, 600.0, 1.5),
        Vector(600.0, 600.0, 1.5),
        Vector(600.0, 1140.0, 1.5),
        Vector(600.0, 60.0, 1.5),
      };
      tp.position = SampleAlongPolyline(route, speed * local, true);
      tp.speedMps = speed;
    ...
    std::vector<Vector> waypoints = {
      Vector(600.0, 60.0, 1.5),
      Vector(600.0, 400.0, 1.5),
      Vector(600.0, 800.0, 1.5),
      Vector(600.0, 1140.0, 1.5),
      Vector(600.0, 60.0, 1.5),
    };
  ...
  double
  RuralDistancePatternB(double t) const
  {
    // 50 s cycle: accelerate 10s, cruise 30s, decel 10s.
    double cycle = 50.0;
    ...
    // Total distance per 50s cycle: 200 (accel) + 900 (cruise) + 200 (decel) = 1300m.
    double base = static_cast<double>(laps) * 1300.0;

    if (phase < 10.0)
    {
      // v = 10 + 2t => dist = 10t + t^2
      return base + 10.0 * phase + phase * phase;
    }
    if (phase < 40.0)
    {
      return base + 200.0 + 30.0 * (phase - 10.0);
    }

    double dt = phase - 40.0;
    // v = 30 - 2dt => dist = 1100 + 30dt - dt^2
    return base + 1100.0 + 30.0 * dt - dt * dt;
  }

  double
  RuralSpeedPatternB(double t) const
  {
  ...
  TrajectoryPoint
  SampleScenario10(const UeRuntime& ue, double t) const
  {
    TrajectoryPoint tp;
    double local = t + ue.phaseOffsetS;
    ...
    // C: emergency U-turn through the coverage hole (go forward, pause, return).
    double cycle = 420.0;
    double phase = std::fmod(local, cycle);
    if (phase < 0.0)
    {
  ...
  TrajectoryPoint
  SampleTrajectory(const UeRuntime& ue, double nowS) const
  {
    TrajectoryPoint tp;
    switch (m_scenario.id)
    {
    ...
    // Apply per-UE jitter and wrap to the scenario box.
    ...
    // Scenario 10: mark coverage hole region.
    if (m_scenario.id == 10)
    {
  ...
  double
  ScenarioInterferenceScale(const TrajectoryPoint& tp) const
  {
    ...
    case 9:
    {
      // Non-stationary interference burst: only affects the downtown box during a time window.
      bool downtown = (tp.position.x >= 350.0 && tp.position.x <= 850.0 && tp.position.y >= 350.0 && tp.position.y <= 850.0);
      bool burst = (now >= 180.0 && now <= 240.0);
      if (downtown && burst)
      {
        return 3.0;
      }
  ...
  double
  ComputeLosProbability(const TrajectoryPoint& tp, const Vector& gnbPos) const
  {
    (void)gnbPos;
    ...
    case 9:
    {
      ...
      // Street grid alignment increases LOS probability.
  ...
  void
  ResolvePathLossModel(const TrajectoryPoint& tp,
                       bool isLos,
                       double& base,
                       double& slope,
                       double& shadowSigma,
                       ...
                       double& extraLossDb) const
  {
    ...
    {
    case 8:
      // Urban micro-ish. Dense center is effectively handled via LOS probability.
      if (isLos)
      {
        base = 132.0;
        slope = 34.0;
    ...
    case 9:
      // Urban with heavier NLOS and turning corners.
      if (isLos)
      {
        base = 136.0;
        slope = 28.0;
    ...
    case 10:
      if (tp.inTunnel)
      {
        // Coverage hole / deep shadow segment.
        base = 140.0;
        slope = 50.0;
        shadowSigma = 10.0;
        fadingSigma = 6.0;
        ...
        extraLossDb += 15.0 + 25.0 * (depth / 400.0); // 15..40 dB
      }
      else
      {
        base = 128.1;
  ...
  std::vector<CellMeasurement>
  MeasureCells(const TrajectoryPoint& tp)
  {
    ...
    for (const auto& g : m_scenario.gnbs)
    {
      ...
      double base = 140.0;
      double slope = 36.0;
      double sigmaShadow = 7.0;
      double sigmaFading = 4.0;
    ...
    for (size_t i = 0; i < out.size(); ++i)
    {
      ...
      for (size_t j = 0; j < out.size(); ++j)
      {
        ...
        interfMw += DbmToMilliwatt(out[j].rsrpDbm);
      }
  ...
  FindMeasurementByCell(const std::vector<CellMeasurement>& meas, uint32_t cellId) const
  {
  ...
  uint32_t
  BestNeighborCell(const std::vector<CellMeasurement>& meas, uint32_t servingCell, double& bestNeighborRsrp) const
  {
  ...
  // -------------------------------------------------------------------------
  // Handover engine and logging
  // -------------------------------------------------------------------------

  void
  WriteEvent(double now,
             const UeRuntime& ue,
  ...
  void
  CheckPingPong(double now, UeRuntime& ue, uint32_t fromCell, uint32_t toCell, TickDecision& decision)
  {
                  ...
                  double bestNeighborRsrp)
  {
    ...
    // RLF detection (T310 = 2 ticks) based on serving RSRP threshold.
    ...
    if (ue.lowRsrpTicks >= kRlfT310Ticks)
    {
      ...
      // Emergency recovery handover to the strongest cell if acceptable.
    ...
    // Standard A3-style candidate tracking.
    ...
    if (ue.candidateCell != 0 && ue.a3HoldMs >= m_cli.tttMs)
    {
      ...
      if (target != nullptr)
      {
        ...
        WriteEvent(now,
                   ...
                   serving.rsrpDbm,
                   target->rsrpDbm,
                   target->rsrpDbm - serving.rsrpDbm,
                   "A3_TTT");
          ...
          const TickDecision& decision)
  {
    ...
    // CQI approximation from serving SINR (simple table).
    int servingCqi = 1;
  ...
  WriteSummary()
  {
    ...
    uint64_t totalUeHo = 0;
    uint64_t totalUeRlf = 0;
    uint64_t totalUePp = 0;
    ...
    out << "{\n";
    out << "  \"scenario_id\": " << m_scenario.id << ",\n";
    out << "  \"scenario_name\": \"" << m_scenario.name << "\",\n";
    out << "  \"pattern\": \"" << m_pattern.code << "\",\n";
    out << "  \"pattern_name\": \"" << m_pattern.name << "\",\n";
    out << "  \"duration_s\": " << m_durationS << ",\n";
    out << "  \"seed\": " << m_cli.seed << ",\n";
    out << "  \"ue_count\": " << m_cli.ueCount << ",\n";
    out << "  \"ttt_ms\": " << m_cli.tttMs << ",\n";
    out << "  \"hys_db\": " << m_cli.hysDb << ",\n";
    out << "  \"total_handovers\": " << totalUeHo << ",\n";
    out << "  \"total_rlf\": " << totalUeRlf << ",\n";
    out << "  \"total_ping_pong\": " << totalUePp << ",\n";
    out << "  \"handover_per_min\": " << std::fixed << std::setprecision(3) << hoPerMin << "\n";
    out << "}\n";
  }
...
main(int argc, char* argv[])
{
  ...
  cmd.AddValue("pattern", "Pattern within scenario: A|B|C (also accepts e.g. 8A, 9B, 10C)", cli.pattern);
  cmd.AddValue("duration", "Simulation duration in seconds (0 uses pattern default)", cli.durationS);
  cmd.AddValue("ueCount", "Number of UEs (share the pattern with random phase offsets)", cli.ueCount);
...
#ifdef _WIN32
  {
    // Keep relative prefixes rooted under phase_3/test_dataset (avoid scattering output).
