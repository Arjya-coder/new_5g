...
class Algorithm1:
    ...
    def step(self, 
        ...
        for i, nid in enumerate(neighbor_ids):
            ...
            # ✅ REMOVED: A5 condition (redundant)
            # ✅ KEPT: Only A3 condition (dominant)
        ...
        # ✅ REMOVED: Complex slope-based margin prediction
        # ✅ KEPT: Simple, clean margin calculation
        margin_pred = {}
        ...
        if MUST_HO:
            if eligible_candidates:
                ...
                if len(neighbor_ids) > 0:
                    # ✅ FIXED: Only select if RSRP > -115 dBm (not magical perfect cell)
...
class RLModule:
    ...
    def build_state_vector(self, algo1_output, time_since_last_ho=0.0, 
        ...
        # ✅ IMPROVEMENT #1: RSRP trend
        if rsrp_prev is not None:
            rsrp_trend = (rsrp - rsrp_prev) / 0.1  # Per 100ms step
    ...
    def compute_reward(self, rlf_event, ping_pong_event, sinr_delta, sinr_current,
        ...
        # Component 1: RLF penalty (HIGH PRIORITY)
        # ✅ IMPROVEMENT #2: Increased to -1.2 (was -0.5)
        if rlf_event:
            r_rlf = -1.2
        else:
            r_rlf = 0.0
        
        # Component 2: Ping-pong penalty (MEDIUM PRIORITY)
        # ✅ IMPROVEMENT #2: Changed to -0.8 (was -1.0, now less than RLF)
        if ping_pong_event:
            r_pp = -0.8
        else:
            r_pp = 0.0
        
        # Component 3: Continuous SINR reward (delta)
        r_sinr_delta = np.clip(sinr_delta * 0.1, -0.2, 0.2)
        
        # ✅ IMPROVEMENT #4: SINR absolute term (add to delta)
        r_sinr_abs = np.clip((sinr_current + 20) / 30 * 0.1, 0, 0.1)
        r_sinr = r_sinr_delta + r_sinr_abs
        
        # Component 4: Handover cost (LOW PRIORITY)
        # ✅ IMPROVEMENT #2: Kept at -0.1 (lowest priority)
        ...
        # Component 6: No-op penalty
        r_noop = 0.0
        if no_op:
            r_noop = -0.01
        
        # ✅ IMPROVEMENT #3: HO frequency penalty (new)
        r_ho_freq = -0.05 * recent_ho_count
        
        r = r_rlf + r_pp + r_sinr + r_ho + r_smooth + r_noop + r_ho_freq
        
        # Baseline offset for usable learning signal
        r = r + 0.1
        r = np.clip(r, -1.0, +1.0)
...
def train_ppo(agent, rl_module, training_env, num_episodes=500, rollout_horizon=2048,
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % 10 == 0:
            print(f"Episode {episode + 1:4d} | Reward: {metrics['total_reward']:8.2f} | "
                  ...
                  f"HO: {metrics['ho_count']:3d} | TTT: {ttt_rule} | HYS: {hys_rule:.1f}")
...
def analyze_parameter_patterns(param_history_file, output_file):
    ...
    for zone, stats in sorted(analysis.items()):
        print(f"{zone}: TTT avg={stats['avg_ttt']:.0f} HYS avg={stats['avg_hys']:.2f}")
